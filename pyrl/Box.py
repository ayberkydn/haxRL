...
class Box:
    def __init__(self, x_left, x_right, y_up, y_down, r):
        horiz_length = x_right - x_left
        vert_length = y_down - y_up
        assert (horiz_length > 0 and vert_length > 0)
        
        self.borders = {
                "up": HorizontalBorder((x_left + x_right) / 2, y_up, horiz_length, r),
                "down": HorizontalBorder((x_left + x_right) / 2, y_down, horiz_length, r),
                "left": VerticalBorder(x_left, (y_up + y_down) / 2, vert_length, r),
                "right": VerticalBorder(x_right, (y_up + y_down) / 2, vert_length, r),
        }
